...
class Resource
{
	...
	public function th($resourceName, $field)
	{
		$config = \AdminHelper::getConfig($resourceName, 'fields.format.'.$field, $field);
	...
	public function td($resourceName, $field, $data, $params = array())
	{
		$config = \AdminHelper::getConfig($resourceName, 'fields.format.'.$field, $field);		
		if(is_string($config)) return $data->$field;
		elseif(is_array($config))
		{
			return call_user_func_array($config[1], $params); 
		}else{
			return $params[1]->$field;
		}
	}
}
